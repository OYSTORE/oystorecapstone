...
const RestaurantPage = ({ restaurant2, resID }) => {
    ...
    useEffect(() => {
        !currentUser ? Router.push("/auth/login"):"";
        
    },[currentUser])
    
    let ratingArray = []
    Object.values(restaurant.reviewLists).map((review) =>
  ...
  const docRef = await addDoc(collectionRef3, {
            ...
            userRef,
            {
                reservations: {
                    [reserveKeyUser]: {
                        // dishID: dish.id,
                        ...data,
                        reservationTo: restaurant.name,
                        userID: currentUser.uid,
                        userName: currentUser.displayName,
                        userProfilePic: currentUser.photoURL,
                        restaurantID: resID,
                        restaurantImg: restaurant.src,
                        userEmail: currentUser.email,
                        reservationStatus: "Pending",
                        reservationKey: reserveKeyUser,
                        restaurantNo: restaurant.contactNumber,
                        restaurantEmail: restaurant.emailAddress,
                    },
                },
            },
            { merge: true }
    ...
    const handleAddReview = async (e) => {
        ...
        let ratingArray2 = []
        Object.values(restaurant.reviewLists).map((review) =>
            ratingArray2.push(review.rating)
        )
        let reviews2
        let newRating
        if (Object.keys(restaurant.reviewLists).includes(currentUser.uid)) {
            let newArrayIncluded = await removeItemOnce(
                ratingArray2,
                userData.reviewLists[resID].rating
            )
            newArrayIncluded.push(currentValue)
            reviews2 = ratingArray2.length
            var sum = 0
            for (var i = 0; i < reviews2; i++) {
                sum += newArrayIncluded[i]
            }
            ...
            restaurantRef,
            {
                reviewLists: {
                    [currentUser.uid]: {
                        ...
                        rating: currentValue,
                        datePublished: currentDateReview,
                            ...
                            saepe minima natus!
                                ...
                                <div className="menu-container">
                                    <div className="flex flex-row justify-between items-center border-b">
                                        ...
                                        <h1
                                            className="text-xl text-orange-peel cursor-pointer font-medium"
                                            onClick={() =>
                                                setToggleModalDish(!toggleModal)
                                            }
                                        >
                                            View All
                                        </h1>
                                ...
                                <div className="reviews-container py-2">
                                    ...
                                    <div className="flex flex-col px-2 py-4 gap-4 ">
                                        {Object.entries(
                                            restaurant.reviewLists
                                        ).map((review) => (
                                            <ReviewCard
                                                key={review[1].reviewerUserID}
                                                review={review[1]}
                                            />
                                        ))}
                                    </div>
                                    ...
                                    <div className="px-3">
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <textarea
                                                ...
                                                onChange={
                                                    handleAddInputChangeReview
                                                }
                                                className="resize-none h-60 rounded-md"
                                            />
                                        </div>
